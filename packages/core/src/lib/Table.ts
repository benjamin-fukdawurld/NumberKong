...
export default class Table {
  ...
  get score(): number {
    return this._values.reduce(
      (accum, current) => accum - (current <= 0 ? 0 : 1),
      0
    );
  }
  ...
  get playableMoves(): Move[] {
    ...
    for (let row = 0, rmax = this.rowCount; row < rmax; ++row) {
      for (let col = 0, cmax = this.colCount; col < cmax; ++col) {
        results.push(...this.checkCell(row, col));
      }
    }
  ...
  init(
    colCount: number,
    rng: Generator<number, void, number> = Table.staticRng()
  ): void {
    this._values = [];
    this._colCount = colCount;
    ...
    while (!current.done) {
      this._values.push(current.value);
      current = rng.next();
    }
  ...
  row(index: number): Row {
    if (index < 0 || !Number.isInteger(index) || index >= this.rowCount) {
      throw new Error(
        `Cannot get row: index must be a valid integer in range [0, ${
          this.rowCount - 1
        }]`
      );
    }
  ...
  setValue(row: number, col: number, value: number): void {
    ...
    if (!Number.isInteger(value)) {
      throw new Error(
        `Cannot set value(${row}, ${col}): invalid value '${value}'`
      );
    }
  ...
  play(move: Move | string): void {
    if (typeof move === 'string') {
      move = Move.fromString(move);
    }

    if (
      move.positions[0].row === move.positions[1].row &&
      move.positions[0].col === move.positions[1].col
    ) {
      throw new Error('Move must be two different cells');
    }

    const cell1 = this.cell(move.positions[0].row, move.positions[0].col);
    const cell2 = this.cell(move.positions[1].row, move.positions[1].col);

    if (cell1.isSolved) {
      throw new Error(
        `cell(${cell1.row.index}, ${cell1.column}) is already solved`
      );
    }

    if (cell2.isSolved) {
      throw new Error(
        `cell(${cell2.row.index}, ${cell2.column}) is already solved`
      );
    }
  ...
  removeRow(index: number): void {
    if (index < 0 || index >= this.rowCount) {
      throw new Error(
        `Cannot remove row n°${index}: invalid index, index must be in range [0, ${
          this.rowCount - 1
        }]`
      );
    }

    const rowStart = index * this.colCount;
  ...
  append(): void {
    ...
    const remaining = this._values.length % this.colCount;
    if (remaining === 0) {
      return;
    }
  ...
  assign(data: string | string[] | number[]): this {
    if (typeof data === 'string') {
      data = data.split(' ');
    }

    const toInt = (val: string | number) => {
      const result = typeof val === 'string' ? parseInt(val) : Math.floor(val);
  ...
  private checkDiagonals(cell: Cell): Move[] {
    const results = [];
    let other = cell.topLeft;
    ...
    other = cell.bottomLeft;
    if (other && cell.match(other)) {
      results.push(new Move([cell.position, other.position]));
    }

    other = cell.bottomRight;
    if (other && cell.match(other)) {
      results.push(new Move([cell.position, other.position]));
    }
  ...
  private checkCross(cell: Cell): Move[] {
    const results = [];
  ...
  private checkCell(row: number, col: number): Move[] {
    const cell = this.cell(row, col);
    if (cell.isSolved) {
      return [];
    }

    return [...this.checkCross(cell), ...this.checkDiagonals(cell)];
  }
